...
class GomokuState(TwoPlayerGameState):
	...
	def make_move(self, action):
		...
		if self.next_player == 1 and self.score1[x][y] >= SHAPE_SCORE["FIVE"]:
			new_board._game_result = 1
		if self.next_player == 2 and self.score2[x][y] >= SHAPE_SCORE["FIVE"]:
			new_board._game_result = 2
		new_board.board[x][y] = self.next_player
		new_board.update_score(x, y, self.next_player)
		new_board._last_player = self.next_player
		if self.next_player == 1:
			new_board.chess1.append((x, y))
		elif self.next_player == 2:
			new_board.chess2.append((x, y))
		return new_board
